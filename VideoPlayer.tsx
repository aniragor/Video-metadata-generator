...
export default function VideoPlayer({
  ...
  const handleMetadata = (e: SyntheticEvent<HTMLVideoElement>) => {
          ...
          <p>
            {isLoadingVideo
              ...
              : videoError
                ...
                : 'Перетащите видеофайл(ы) сюда, чтобы начать.'}
          </p>
        </div>
      )}
      <div className="videoActions">
        <button className="button uploadButton" onClick={onUploadClick}>
          <span className="icon">upload</span> Загрузить
        </button>
        {hasVideos && (
          <button className="button clearButton" onClick={onClearAll}>
            <span className="icon">delete_sweep</span> Очистить все
          </button>
        )}
      </div>
    </div>
